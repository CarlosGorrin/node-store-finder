...
function loadPlaces(map) {
	...
	_axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
		...
		//when someone clicks on a marker, show the details og that place
		...
		// the zoom thw map to fit all the markers perfectly
...
function typeAhead(search) {
	...
	searchInput.on('input', function () {
		...
		_axios2.default.get('/api/search?q=' + this.value).then(function (res) {
			...
			searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No value for ' + _this.value + ' found!</div>');
		}).catch(function (err) {
			console.error(err);
		});
	});

	//hanfle keyboard inputs
	searchInput.on('keyup', function (e) {
		// if tey aren't pressing up, down or enter, who cares!
